...
function getIntersectionPoints(path1, path2) {
	...
	for(p of path1){
		if(path2.includes(p)){
			intersectionPoints.push(p.split('.'));
		}
	}	
...
let w1_path = getPath(wire1);
let w2_path = getPath(wire2);
let w1_path_set = new Set(w1_path.keys());
let w2_path_set = new Set(w2_path.keys());
var intersectionPoints = new Set([...w1_path_set].filter(x => w2_path_set.has(x)));

let minSteps = Infinity;
for(let point of intersectionPoints){
	let steps = w1_path.get(point) + w2_path.get(point);
	if(steps < minSteps){
		minSteps = steps;
	}
}
